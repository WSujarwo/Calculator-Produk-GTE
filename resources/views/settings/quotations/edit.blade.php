    ...
    <div class="w-full px-6 lg:px-10 py-6">
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200/40 p-8">
            <form action="{{ route('settings.quotations.update', $quotation) }}" method="POST" class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700">Quotation No</label>
                    <input type="text" value="{{ $quotation->quotation_no }}" disabled
                           class="mt-1 w-full rounded-xl border-gray-300 bg-gray-100 text-gray-500">
                </div>
                    ...
                    <select name="company_id" id="companySelect"
                        ...
                        @foreach ($companies as $company)
                            <option value="{{ $company->id }}"
                                @selected(old('company_id', $quotation->company_id) == $company->id)>
                    ...
                    <select name="marketing_id" id="marketingSelect"
                        ...
                        @foreach ($marketings as $marketing)
                            <option value="{{ $marketing->id }}"
                                @selected(old('marketing_id', $quotation->marketing_id) == $marketing->id)>
