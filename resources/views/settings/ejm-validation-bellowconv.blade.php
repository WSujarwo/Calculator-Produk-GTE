<x-app-layout>
    <x-slot name="header">
        <div class="px-6 lg:px-10 flex items-start justify-between gap-3">
            <div>
                <h2 class="font-bold text-2xl text-gray-900 leading-tight">Validasi Bellowconv EJM</h2>
                <p class="text-sm text-gray-600">Kelola tabel <code>validasi_dataejm_bellowconvs</code>.</p>
            </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
            ...
            <section class="xl:col-span-9 rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-4 py-3">
                    <form method="GET" action="{{ route('setting.ejm-validation-bellowconv.index') }}"
                          class="grid grid-cols-1 md:grid-cols-4 gap-3">
                        <label class="text-xs text-slate-600">
                            <span class="font-semibold">Size</span>
                            <input type="number" name="size" value="{{ request('size') }}"
                                   class="mt-1 w-full rounded-lg border-slate-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </label>
                        <label class="text-xs text-slate-600">
                            <span class="font-semibold">NOC</span>
                            <input type="number" name="noc" value="{{ request('noc') }}"
                                   class="mt-1 w-full rounded-lg border-slate-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </label>
                        <label class="text-xs text-slate-600">
                            <span class="font-semibold">Naming</span>
                            <input type="text" name="naming" value="{{ request('naming') }}"
                                   class="mt-1 w-full rounded-lg border-slate-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </label>
                        <div class="flex items-end gap-2">
                            ...
                            <a href="{{ route('setting.ejm-validation-bellowconv.index') }}"
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
                    <h3 class="font-semibold text-slate-900">Data Validasi Bellowconv</h3>
                    <div class="flex flex-wrap gap-2">
                        @can('settings.ejm-validation.create')
                            <a href="{{ route('setting.ejm-validation-bellowconv.create') }}"
                        ...
                        @can('settings.ejm-validation.export')
                            <a href="{{ route('setting.ejm-validation-bellowconv.template.csv') }}"
                               class="inline-flex items-center rounded-lg bg-slate-700 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800">
                                Template CSV
                            </a>
                            <a href="{{ route('setting.ejm-validation-bellowconv.template.excel') }}"
                               class="inline-flex items-center rounded-lg bg-slate-700 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800">
                                Template Excel
                            </a>
                            <a href="{{ route('setting.ejm-validation-bellowconv.export.csv', request()->query()) }}"
                               class="inline-flex items-center rounded-lg bg-amber-600 px-3 py-2 text-sm font-semibold text-white hover:bg-amber-700">
                                Export CSV
                            </a>
                            <a href="{{ route('setting.ejm-validation-bellowconv.export.excel', request()->query()) }}"
                               class="inline-flex items-center rounded-lg bg-amber-600 px-3 py-2 text-sm font-semibold text-white hover:bg-amber-700">
                                Export Excel
                            </a>
                        @endcan
                ...
                <div class="overflow-x-auto overflow-y-auto max-h-[70vh]">
                    <table class="min-w-[900px] text-sm border-collapse">
                        <thead class="sticky top-0 z-10">
                            <tr class="bg-emerald-100 text-slate-900">
                                ...
                                <th class="px-3 py-2 border border-slate-300">NOC</th>
                                <th class="px-3 py-2 border border-slate-300">Naming</th>
                                <th class="px-3 py-2 border border-slate-300">OALB (mm)</th>
                                <th class="px-3 py-2 border border-slate-300">BL (mm)</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($rows as $row)
                                <tr class="hover:bg-slate-50">
                                    <td class="px-3 py-2 border border-slate-200 whitespace-nowrap">
                                        @can('settings.ejm-validation.edit')
                                            <a href="{{ route('setting.ejm-validation-bellowconv.index', array_merge(request()->query(), ['edit' => $row->id])) }}"
                                        ...
                                        @can('settings.ejm-validation.delete')
                                            <form method="POST" action="{{ route('setting.ejm-validation-bellowconv.delete') }}" class="inline">
                                                @csrf
                                    ...
                                    <td class="px-3 py-2 border border-slate-200">{{ $row->size }}</td>
                                    <td class="px-3 py-2 border border-slate-200">{{ $row->noc }}</td>
                                    <td class="px-3 py-2 border border-slate-200">{{ $row->naming }}</td>
                                    <td class="px-3 py-2 border border-slate-200">{{ $row->oalb_mm }}</td>
                                    <td class="px-3 py-2 border border-slate-200">{{ $row->bl_mm }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td class="px-3 py-6 text-sm text-slate-500 border border-slate-200 text-center" colspan="6">
                                        Belum ada data validasi bellowconv.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <div id="createModal" class="fixed inset-0 z-50 {{ $openCreateModal ? 'flex' : 'hidden' }} items-center justify-center bg-black/40 px-4">
        <div class="w-full max-w-4xl rounded-2xl bg-white p-6 shadow-xl">
            <div class="mb-4 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-slate-900">Create Validasi Bellowconv</h3>
                <a href="{{ route('setting.ejm-validation-bellowconv.index') }}" class="text-slate-500 hover:text-slate-700">x</a>
            </div>
            <form method="POST" action="{{ route('setting.ejm-validation-bellowconv.store') }}" class="grid grid-cols-1 md:grid-cols-3 gap-3">
                @csrf
                @foreach ([
                    ['size', null],
                    ['noc', null],
    ...
    <div id="editModal" class="fixed inset-0 z-50 {{ $editing ? 'flex' : 'hidden' }} items-center justify-center bg-black/40 px-4">
        <div class="w-full max-w-4xl rounded-2xl bg-white p-6 shadow-xl">
            <div class="mb-4 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-slate-900">Edit Validasi Bellowconv</h3>
                <a href="{{ route('setting.ejm-validation-bellowconv.index', request()->query()) }}" class="text-slate-500 hover:text-slate-700">x</a>
            </div>
            @if ($editing)
                <form method="POST" action="{{ route('setting.ejm-validation-bellowconv.update') }}" class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    @csrf
                    <input type="hidden" name="id" value="{{ $editing['id'] }}">
                    @foreach (['size', 'noc', 'naming', 'oalb_mm', 'bl_mm'] as $field)
                        <label class="text-xs text-slate-600">
                            <span class="font-semibold">{{ $field }}</span>
                    ...
                    @endforeach
                    <div class="md:col-span-3 flex justify-end gap-2 pt-2">
                        <a href="{{ route('setting.ejm-validation-bellowconv.index', request()->query()) }}"
                           class="rounded-lg bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-200">Cancel</a>
                        <button type="submit"
    ...
    <div id="importModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 px-4">
        <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
            <div class="mb-4 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-slate-900">Import Validasi Bellowconv</h3>
                <button type="button" data-close-modal="importModal" class="text-slate-500 hover:text-slate-700">x</button>
            </div>
            <p class="mb-3 text-sm text-slate-600">Key update import: kombinasi <b>size + noc</b>.</p>
            ...
            <form method="POST" action="{{ route('setting.ejm-validation-bellowconv.import') }}" enctype="multipart/form-data" class="space-y-3">
                @csrf
