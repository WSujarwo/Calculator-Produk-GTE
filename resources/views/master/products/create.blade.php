<x-app-layout>
    <x-slot name="header">
        <div class="px-6 lg:px-10 flex items-center justify-between">
            <div>
                <h2 class="font-bold text-2xl text-white leading-tight">Create Product</h2>
                <p class="text-sm text-gray-300">Tambah data product</p>
            </div>
    ...
    <div class="w-full px-6 lg:px-10 py-6 space-y-4">
        ...
        <div class="rounded-2xl bg-white shadow-lg border border-gray-200/50 p-6">
            <form method="POST" action="{{ route('master.products.store') }}" class="grid grid-cols-1 md:grid-cols-12 gap-4">
                @csrf

                <div class="md:col-span-4">
                    <label class="text-xs font-semibold text-gray-600">Product Code</label>
                    <input name="product_code" value="{{ old('product_code') }}" required
                           class="mt-1 w-full rounded-xl border-gray-200 focus:border-indigo-400 focus:ring-indigo-300"
                           placeholder="EJM" />
                    <p class="text-xs text-gray-500 mt-1">Contoh: EJM, RTI, GPP</p>
                </div>
                ...
                <div class="md:col-span-6">
                    ...
                    <input name="product_name" value="{{ old('product_name') }}" required
                           class="mt-1 w-full rounded-xl border-gray-200 focus:border-indigo-400 focus:ring-indigo-300"
                           placeholder="Expansion Joint Metal (EJM)" />
                </div>
