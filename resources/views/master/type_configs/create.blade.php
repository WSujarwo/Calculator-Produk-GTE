<x-app-layout>
    <x-slot name="header">
        <div class="px-6 lg:px-10">
            <h2 class="font-bold text-2xl text-gray-900 leading-tight">Create Type / Configuration</h2>
            <p class="text-sm text-gray-600">Tambah type config</p>
        </div>
    </x-slot>
    ...
    <div class="w-full px-6 lg:px-10 py-6 space-y-4 text-gray-900">
        ...
        <div class="rounded-2xl bg-white shadow-lg border border-gray-200/50 p-6">
            <form method="POST" action="{{ route('master.type-configs.store') }}"
                ...
                <div class="md:col-span-4">
                    ...
                    <select name="shape_id"
                        ...
                        @foreach($shapes as $s)
                            <option value="{{ $s->id }}" @selected(old('shape_id')==$s->id)>
                                {{ $s->shape_code }} - {{ $s->shape_name }}
                            </option>
                        @endforeach
                    </select>
                    <p class="text-xs text-gray-500 mt-1">Kosongkan jika type tidak tergantung shape.</p>
                </div>
                ...
                <div class="md:col-span-4">
                    ...
                    <input name="type_code" value="{{ old('type_code') }}" required
                           placeholder="SINGLE_UNREINFORCED_UNTIED"
                           class="mt-1 w-full rounded-xl border-gray-200 focus:border-indigo-400 focus:ring-indigo-300 text-gray-900" />
                </div>

                <div class="md:col-span-6">
                    <label class="text-xs font-semibold text-gray-700">Type Name</label>
                    <input name="type_name" value="{{ old('type_name') }}" required
                           placeholder="Single Unreinforced-Untied Expansion Joint"
                           class="mt-1 w-full rounded-xl border-gray-200 focus:border-indigo-400 focus:ring-indigo-300 text-gray-900" />
                </div>
                ...
                <div class="md:col-span-12">
                    ...
                    <textarea name="notes" rows="3"
                              class="mt-1 w-full rounded-xl border-gray-200 focus:border-indigo-400 focus:ring-indigo-300 text-gray-900">{{ old('notes') }}</textarea>
                </div>
