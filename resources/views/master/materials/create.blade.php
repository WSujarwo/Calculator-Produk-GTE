<x-app-layout>
    <x-slot name="header">
        <div class="px-6 lg:px-10">
            <h2 class="font-bold text-2xl text-gray-900 leading-tight">Create Material</h2>
            <p class="text-sm text-gray-600">Tambah data material manual</p>
        </div>
    </x-slot>

    <div class="w-full px-6 lg:px-10 py-6 text-gray-900">
        <div class="rounded-2xl bg-white shadow-lg border border-gray-200/50 p-5">
            <form method="POST" action="{{ route('master.materials.store') }}" class="grid grid-cols-1 md:grid-cols-12 gap-4">
                @csrf
                ...
                <div class="md:col-span-4">
                    ...
                    <input type="text" name="naming" value="{{ old('naming') }}"
                        class="mt-1 w-full rounded-xl border-gray-200 focus:border-indigo-400 focus:ring-indigo-300 text-gray-900" />
                    @error('naming') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                </div>

                <div class="md:col-span-4">
                    <label class="text-xs font-semibold text-gray-700">Quality</label>
                    <input type="text" name="quality" value="{{ old('quality') }}"
                        class="mt-1 w-full rounded-xl border-gray-200 focus:border-indigo-400 focus:ring-indigo-300 text-gray-900" />
                ...
                <div class="md:col-span-12">
                    ...
                    <textarea name="description" rows="3"
                        class="mt-1 w-full rounded-xl border-gray-200 focus:border-indigo-400 focus:ring-indigo-300 text-gray-900">{{ old('description') }}</textarea>
                </div>
