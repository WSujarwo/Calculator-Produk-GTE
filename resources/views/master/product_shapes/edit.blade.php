<x-app-layout>
    <x-slot name="header">
        <div class="px-6 lg:px-10">
            <h2 class="font-bold text-2xl text-gray-900 leading-tight">Edit Mapping</h2>
            <p class="text-sm text-gray-600">Ubah mapping Product ↔ Shape</p>
        </div>
    </x-slot>
    ...
    <div class="w-full px-6 lg:px-10 py-6 space-y-4 text-gray-900">
        ...
        <div class="rounded-2xl bg-white shadow-lg border border-gray-200/50 p-6">
            <form method="POST" action="{{ route('master.product-shapes.update', $mapping) }}"
                ...
                <div class="md:col-span-5">
                    ...
                    <select name="product_id" required
                        ...
                        @foreach($products as $p)
                            <option value="{{ $p->id }}" @selected(old('product_id', $mapping->product_id)==$p->id)>
                ...
                <div class="md:col-span-5">
                    ...
                    <select name="shape_id" required
                            class="mt-1 w-full rounded-xl border-gray-200 focus:border-indigo-400 focus:ring-indigo-300 text-gray-900">
                        @foreach($shapes as $s)
                            <option value="{{ $s->id }}" @selected(old('shape_id', $mapping->shape_id)==$s->id)>
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="is_active"
                            class="mt-1 w-full rounded-xl border-gray-200 focus:border-indigo-400 focus:ring-indigo-300 text-gray-900">
                        <option value="1" @selected(old('is_active', (int)$mapping->is_active)===1)>Yes</option>
                        <option value="0" @selected(old('is_active', (int)$mapping->is_active)===0)>No</option>
                    </select>
                </div>
